...
class EfficientMixer(nn.Module):
    """
    ...
    def __init__(self, in_dims=256):
        super().__init__()

        self.to_query = nn.Linear(in_dims, in_dims)
        self.to_key = nn.Linear(in_dims, in_dims)

        self.w_g = nn.Parameter(torch.randn(in_dims, 1)).requires_grad_(False)
        self.scale_factor = in_dims ** -0.5
        self.Proj = nn.Linear(in_dims, in_dims)
        self.final = nn.Linear(in_dims, in_dims)
    ...
    def forward(self, x):
        ...
        out = self.Proj(A*key) + value #BxNxD

        out = self.final(out) # BxNxD

        return out

class EM_TransformerEncoderLayer(nn.Module):
    def __init__(
            self,
            d_model=256,
            d_ffn=1024,
        ...
        # self attention
        self.efficient_attn = EfficientMixer(d_model)
...
class EM_DeformerTransformer(nn.Module):
    def __init__(
            self,
            temp=10000,
            d_model=256,
        ...
        self.em_encoder = EM_TransformerEncoderLayer(d_model)
        ...
        self.catmull_coord_embed= None
        self.catmull_class_embed= None
        self.enc_output = nn.Linear(d_model, d_model)
        self.enc_output_norm = nn.LayerNorm(d_model)
        self.num_points = num_points

        t_values = torch.linspace(0, 1, num_points) 
        self.basis_matrix = torch.stack([self.catmull_rom_basis(t) for t in t_values], dim=0)  
    ...
    def init_points_from_rom_proposals(self, reference_catmull):
        bz = reference_catmull.shape[0]
        initial_reference_points = reference_catmull.view(bz, self.num_proposals, 4, 2)
        initial_reference_points = torch.matmul(
            upcast(self.basis_matrix.to(initial_reference_points.device)),
    ...
    def gen_encoder_output_proposals(self, memory, memory_padding_mask, spatial_shapes):
        ...
        mask = (output_proposals > 0.01) & (output_proposals < 0.99)
        output_proposals_valid = mask.prod(dim=-1, keepdim=True).bool()
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        # use em_encoder
        src_flatten = self.em_encoder(src_flatten)
        memory = self.encoder(
            src_flatten,
            spatial_shapes,
            level_start_index,
        ...
        enc_outputs_class = self.catmull_class_embed(output_memory)
        enc_outputs_coord_unact = self.catmull_coord_embed(output_memory) + output_proposals
        ...
        reference_points = self.init_points_from_rom_proposals(reference_points)
