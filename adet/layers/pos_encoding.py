...
class PositionalEncoding2D(nn.Module):
    ...
    def forward(self, tensors):
        ...
        y_embed = torch.arange(1, H + 1, device=x.device, dtype=torch.float32)  
        y_embed = y_embed.view(1, H, 1)  
        y_embed = y_embed.expand(B, H, W)  

        x_embed = torch.arange(1, W + 1, device=x.device, dtype=torch.float32)
        x_embed = x_embed.view(1, 1, W) 
        x_embed = x_embed.expand(B, H, W) 
