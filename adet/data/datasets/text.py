...
"""
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        ...
        for anno in anno_dict_list:
            ...
            catrompts = anno.get("catmullrom_pts", None)
            if catrompts:
                catrompts = np.array(catrompts).reshape(-1, 2)
                center_catrompts = (catrompts[:4] + catrompts[4:][::-1, :]) / 2
                obj["catroms"] = center_catrompts
                catrompts = catrompts.reshape(2, 4, 2).transpose(0, 2, 1).reshape(4, 4) # [[x1, x]]
                u = np.linspace(0, 1, num_pts_cfg) 
                u2 = u**2
                u3 = u**3
                boundary = np.outer(-0.5 * u3 + u2 - 0.5 * u, catrompts[:, 0]) \
                    + np.outer(1.5 * u3 - 2.5 * u2 + 1.0, catrompts[:, 1]) \
                    + np.outer(-1.5 * u3 + 2.0 * u2 + 0.5 * u, catrompts[:, 2]) \
                    + np.outer(0.5 * u3 - 0.5 * u2 , catrompts[:, 3])
