...
def transform_instance_annotations(
    ...
    if "catroms" in annotation:
        catroms = transform_ctrl_pnts_annotations(annotation["catroms"], transforms)
        annotation["catroms"] = catroms
...
def transform_ctrl_pnts_annotations(pnts, transforms):
    ...
    Args:
        catroms (list[float]): Nx16 float in Detectron2 Dataset format.
        transforms (TransformList):
    """
    # (N*2,) -> (N, 2)
    pnts = np.asarray(pnts, dtype="float32").reshape(-1, 2)
...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    # add attributes
    if "catroms" in annos[0]:
        catroms = [obj.get("catroms", []) for obj in annos]
        instance.catroms = torch.as_tensor(catroms, dtype=torch.float32)
