...
from adet.utils.curve_utils import CatromSampler
...
class CatromHungarianMatcher(nn.Module):
    def __init__(
            self,
            class_weight: float = 1,
            coord_weight: float = 1,
        ...
        Params:
            ...
            coord_weight: not the control points of catrom curve but the sampled points on curve,
        ...
        self.gamma = focal_gamma
        assert class_weight != 0 or coord_weight != 0, "all costs cant be 0"
        self.catrom_sampler = CatromSampler(num_sample_points=num_sample_points)

    def forward(self, outputs, targets):
        with torch.no_grad():
            bs, num_queries = outputs["pred_logits"].shape[:2]
            out_prob = outputs["pred_logits"].flatten(0, 1).sigmoid()
            out_catroms = outputs["pred_catroms"].flatten(0, 1).view(-1, 4, 2)  # (batch_size * num_queries, 4, 2)
            tgt_ids = torch.cat([v["labels"] for v in targets])
            tgt_catroms = torch.cat([v["catroms"] for v in targets])  # (g, 4, 2)
            ...
            # Compute the L1 cost betweeen sampled points on Catmull-Rom curve
            cost_coord = torch.cdist(
                (self.catrom_sampler.get_sample_points(out_catroms)).flatten(start_dim=-2),
                (self.catrom_sampler.get_sample_points(tgt_catroms)).flatten(start_dim=-2),
                p=1
            )
            C = self.class_weight * cost_class + self.coord_weight * cost_coord
            C = C.view(bs, num_queries, -1).cpu()

            sizes = [len(v["catroms"]) for v in targets]
            indices = [
...
def build_matcher(cfg):
    ...
    return CatromHungarianMatcher(class_weight=cfg.CATMULL_CLASS_WEIGHT,
                                  coord_weight=cfg.CATMULL_COORD_WEIGHT,
                                  num_sample_points=cfg.CATMULL_SAMPLE_POINTS,
