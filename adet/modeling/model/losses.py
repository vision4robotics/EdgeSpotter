...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_inst, log=False):
        ...
        # print("taro", target_classes_o.shape)
    ...
    def loss_catroms(self, outputs, targets, indices, num_inst):
        # may FIX: (1) seg valid points
        assert 'pred_catroms' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_catroms = outputs['pred_catroms'][idx]
        src_catroms =  self.catrom_sampler.get_sample_points(src_catroms.view(-1, 4, 2))
        target_catroms = torch.cat(
            [t['catroms'][i] for t, (_, i) in zip(targets, indices)],
            dim=0
        )
        target_catroms = self.catrom_sampler.get_sample_points(target_catroms)
        if target_catroms.numel() == 0:
            target_catroms = src_catroms.clone().detach()
        loss_catrom = F.l1_loss(src_catroms, target_catroms, reduction='none')
        losses = {}
        losses['loss_catrom'] = loss_catrom.sum() / num_inst
        return losses
