...
from adet.layers.em_transformer import EM_DeformerTransformer
...
class DETECTION_TRANSFORMER(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        self.transformer = EM_DeformerTransformer(
        ...
        # catrom center line proposal after the encoder
        # x_0, y_0, ... , x_3, y_3
        self.catmull_proposal_coord = MLP(self.d_model, self.d_model, 8, 3)
        self.catmull_proposal_class = nn.Linear(self.d_model, self.num_classes)  # text or non-text
        ...
        self.catmull_proposal_class .bias.data = torch.ones(self.num_classes) * bias_value
        ...
        nn.init.constant_(self.catmull_proposal_coord.layers[-1].weight.data, 0)
        nn.init.constant_(self.catmull_proposal_coord.layers[-1].bias.data, 0)
        self.transformer.catmull_coord_embed = self.catmull_proposal_coord
        self.transformer.catmull_class_embed = self.catmull_proposal_class 
    ...
    def forward(self, samples: NestedTensor):
        ...
        # print("enc_outputs_coord:", enc_outputs_coord.shape)
        out['enc_outputs'] = {
            'pred_logits': enc_outputs_class,
            'pred_catroms': enc_outputs_coord
        }
